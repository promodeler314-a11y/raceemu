...
**ロードマップ（`docs/roadmap.md`）だけを見て手を付けないこと。** 個別の設計書が順序の
制約を書いている場合があり、実際に食い違っていた。順位条件の既定化はロードマップでは
「1 行の変更」だが、`docs/order-field.md` の 7 節は束の作り方を直したあとにやることだと
書いている。着手前に、その機能の設計書の段取りを読む。
