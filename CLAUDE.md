...
pnpm test                      # vitest run。27 ファイル 240 件
...
240 件のうち 13 件は手元の材料に依り、無ければ静かに飛ぶ。**緑でも全部通ったとは限らない。**
...
2. **フィールド**（`field/field.ts`）：他馬の位置の時系列を先に生成して渡す。**相手同士は一緒に走らせる**（隊列ができないと順位が塊の境目にしか出ない）が、自分の走りは相手に影響しないので、束は全試行と全候補で使い回せる。フィールドを渡すと、自分の位置取りも近似から束の先頭に対する判定に切り替わり、出走前は順位条件を判定しなくなる。**相手は自分と同格の分布である**（束の 1 本ごとに脚質構成・強さ・やる気・スキルを引き直す。`defaultFieldProfile`）。引き直しは鍵から決まるので、同じ指定なら同じ束になる。ここを崩すと共通乱数によるペア比較が成り立たない。作り方ごとの比較は `docs/order-field.md` の 3 節にある。
...
**「いま何が残っているか」は [issue](https://github.com/promodeler314-a11y/raceemu/issues) だけが持つ。**
以前は README と `docs/roadmap.md`、`docs/ui-gap.md`、`docs/server-design.md` が同じ項目を
別の言葉で持っていて、どれが最新か分からなくなった。設計書は「なぜそうするか」を持つ。

束はラベルで分かれている。`needs-measurement`（ゲームからの実測が要る）、`quick-win`、
`major`、`later` である。**いちばん重要なのは `needs-measurement` で、コードでは閉じない。**
順位条件のフィールドはゲームと突き合わせておらず、バ身の換算（1 バ身 = 2.5 m）と順位率の
対応表もスキルデータの注記からの読み取りである。

`docs/` の設計書は、決めたことを先に書いてコードが後から追いつく形で運用している。
**ロードマップだけを見て手を付けないこと。** 個別の設計書が順序の制約を書いている場合があり、
実際に食い違っていた。順位条件の既定化はロードマップでは「1 行の変更」だが、
`docs/order-field.md` の 7 節は束の作り方を直したあとにやることだと書いている。
着手前に、その機能の設計書の段取りを読む。
