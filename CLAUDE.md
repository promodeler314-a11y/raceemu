...
2. **フィールド**（`field/field.ts`）：他馬の位置の時系列を先に生成して渡す。**相手同士は一緒に走らせる**（隊列ができないと順位が塊の境目にしか出ない）が、自分の走りは相手に影響しないので、束は全試行と全候補で使い回せる。フィールドを渡すと自分の位置取りも近似から先頭への判定に切り替わる（`withFieldPositionKeep`）。作り方ごとの比較は `docs/order-field.md` の 3 節にある。
