...
2. **フィールド**（`field/field.ts`）：他馬の位置の時系列を先に生成して渡す。**相手同士は一緒に走らせる**（隊列ができないと順位が塊の境目にしか出ない）が、自分の走りは相手に影響しないので、束は全試行と全候補で使い回せる。フィールドを渡すと、自分の位置取りも近似から束の先頭に対する判定に切り替わり、出走前は順位条件を判定しなくなる。**相手はまだ固定の弱い想定（スキル無し）である。** 作り方ごとの比較は `docs/order-field.md` の 3 節にある。
