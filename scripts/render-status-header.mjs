...
/**
 * 8 階級すべてを 1 枚に入れる。実機 3 枚を合わせても、1 枚には出そろわない。
 *
 * 色は実機で測った色相に合わせる（`apps/api/src/status-reader.ts` の
 * `RANK_HUES`）。色で階級を決める道筋を、見本でも通すためである。
 */
export const SAMPLE_APTITUDES = ['S', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'A', 'S'];

/** 実機で測った色相。G だけは色が付いていない（灰色）。 */
const RANK_COLORS = {
  S: 'hsl(42, 85%, 45%)',
  A: 'hsl(24, 85%, 45%)',
  B: 'hsl(342, 80%, 50%)',
