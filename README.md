...
- **順位条件のフィールドを、相手同士を一緒に走らせる形にした。** 以前は 1 頭ずつ独立に走らせていたので隊列ができず、順位が塊の境目にしか現れなかった。自分も束の先頭に対して位置取りする。順位の分布が全頭同時（勝率の面）とほぼ同じ形になる（[順位条件の判定を実戦に近づける](docs/order-field.md)の 3 節と 7 節）。
- **順位条件の判定を既定で入れた。** M6 で入れた最大の精度改善が既定で切られていたため、初めて使う人は本家と同じ「順位条件は満たしている前提」の甘い評価を見ていた。探索もこの設定に従うので、過大評価を突く状態が既定になっていた（[逆算と探索の設計](docs/solver-design.md)の 4 節）。上の組み替えと同時に入れている。
