...
- **設定**：ステータス、コース、脚質、やる気、バ場状態、キャラ、所持スキル、デバフ、実行オプション。補正後の値とコース形状を併せて出す。固有と進化はキャラから取る。スキル画面の写真から所持スキルを取り込める（サーバを立てた版のみ）。
...
- **勝率**：出走する全頭を同時に走らせる。相手 1 頭ずつにステータスとスキルを持たせ、着順の分布と勝率を出す。
...
```
pnpm install
pnpm test        # 本家との突き合わせとスキル条件の網羅。12 ファイル 112 件
pnpm typecheck
...
- **相手のプリセット。** 勝率の面では相手を 1 頭ずつ入力する。チャンピオンズミーティングの想定を丸ごと当てる口はまだ無い。
- **本物の画面での読み取りの精度。** [M10](docs/m10-report.md) の検査に使っている画像は組み立てたものであり、ゲームの画面ではない（[M10](docs/m10-report.md) の 5 節）。
