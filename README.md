...
- **探索**：目標から必要なステータスを求める逆算と、スキルポイントの予算内で最良の組み合わせを探す探索。逆算は、位置取り調整が何回起きた場合かを分けて出せる。組み合わせ探索の候補は、いま選んでいるスキルからでも、育成計画（育成ウマ娘とデッキと継承）からでも、買えるスキル全体からでも作れる。
全体からにすると候補が数百になるので、サーバの宛先を入れてそちらに投げられる（宛先が無ければ今までどおりブラウザで回る）。
