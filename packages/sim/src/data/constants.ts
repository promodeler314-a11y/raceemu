 ...
 * 「近く」と見なす前後の距離（メートル）。
 *
 * レーンを持たないので、他のウマ娘が「すぐ前」「すぐ後ろ」「近く」にいるかは
 * コース上の位置の差だけで決める。その境目が何メートルなのかは分かっていない。
 * 1 バ身の換算（`bashinMeters`）と同じ性質の未確認項目である。
 *
 * 1 バ身（2.5 m）を採った理由は二つある。どちらも**ゲーム内での実測ではない**。
 *
 * 1. 本家の `near_count` の近似（`skill/approximate.ts`）が置いている分布の
 *    期待値は、序盤 2.5 頭、中盤 1.0 頭、終盤 1.6 頭である。東京 芝2400m の
 *    既定の相手でこの距離を振って数えると、2.5 m のとき 序盤 2.8-3.4 頭、
 *    中盤 0.9-1.3 頭、終盤 0.9-1.5 頭になり、いちばん近い。5 m にすると
 *    どのフェーズもおよそ倍になる。
 * 2. スキルの注記は「すぐ後ろのウマ娘との差が1バ身以内」（レッドエース）のように、
 *    「すぐ前」「すぐ後ろ」を 1 バ身で書くことがある。別の条件型（`bashin_diff_behind`）
 *    の話ではあるが、同じ言葉づかいを同じ距離で読むのは無理が無い。
 *
 * 振ったときに結果がどれだけ動くかは `pnpm sensitivity --near 1.25,2.5,5` で測れる。
 * docs/order-condition.md 5.3 節と docs/order-field.md 8 節を参照。
 */
export const nearLaneMeters = 2.5;
