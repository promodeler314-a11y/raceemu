...
git は消しても履歴に残るので、作り直した回数ぶんの数十 MB は clone に永久に載る。
本家の版が 2〜4 週ごとであることを考えると、年に 10〜25 回積むことになる。
