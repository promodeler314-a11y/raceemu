...
**「組み合わせ探索が重い」の報告を受けて、Worker の使い方を見直した。**
`Evaluator.evaluate` は 1 回の評価につき `pool.run` を 1 回呼ぶだけで、
その塊の大きさが既定の 256 のままだった。200 試行や 600 試行の段は
1 塊に収まってしまい、Worker が何台いても 1 台しか働かない。
`spreadChunkSize`（`packages/solver/src/optimize.ts`）で塊を
Worker 数で割った大きさにし、1 回の評価だけで全 Worker に散らばるようにした。
4 コア（Worker 3 台）の環境で同じ 600 pt を測ると、レース数・評価回数・
選ぶ構成は変わらないまま 109.5 秒 → 69.6 秒になった。
Worker 数の多い環境ほど効きが大きいはずである。
...
M7 の時点での記述である。以後に入ったものには印を付けた。

- ~~**候補の自動的な絞り込み**：いまは画面で選んだスキルがそのまま候補になる。1000 個から選ばせることはできない。~~
  → 育成計画から候補を組み立てる形で入った（[逆算と組み合わせ探索の設計](solver-design.md)の 7 節）。
  レース前に値が決まる条件で先に落とすので、買える 1784 個が東京 芝2400m の先行で 999 個になる。
  全件を候補にするのは[#59](https://github.com/promodeler314-a11y/raceemu/issues/59)に残っている。
- ~~**ヒントレベル**：`cost.ts` は割引を計算できるが、画面から入力する口がない。~~ → 入った。
- **金スキルの前提条件**：上位スキルを取るのに下位が要ることは費用に織り込んであるが、取得順の制約までは見ていない。
- **順位以外の条件**：距離差は[M6](m6-report.md)のフィールド軌跡モデルで判定できるようになった。
  内外の位置（レーン）は未実装のままである（[#55](https://github.com/promodeler314-a11y/raceemu/issues/55)）。
